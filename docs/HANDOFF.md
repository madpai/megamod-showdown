...
- `main` should be at `3cf8e96` (hitch timing in SEND REPORT), published
  ...
  `megamod/claude/vibrant-euler-xwhpy5` up to **3cf8e96 only**, see below).
- The owner may have sent a SEND REPORT from a solo Blood Gulch round over
  the weekend: `scratch/serve-megamod/reports/`. Read the newest with the
  read-report skill. **Check it is build 3cf8e96 and a solo match**
  (`net.hosting` false and no `id` means solo; an `id` means it joined
