...
**Categories:** `character`, `weapon`, `projectile`, `ability`, `vehicle`, `material`, `sound`, `game_mode`, `world_entity`, `mutator`, plus world/map and animation/skeleton assets. Every definition needs a category, canonical ID, owner package ID/version, schema version and source provenance. Optional behavior script IDs belong only to definitions that actually use scripts. The initial [N2 grammar recommendation](CONTENT_ID_GRAMMAR_RECOMMENDATION.md) is `namespace:type/name`, subject to a validation experiment; no parser grammar has been adopted yet. An ID is stable across load-order changes and never derived from a file position.
